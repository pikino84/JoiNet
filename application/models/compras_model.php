...
class Compras_model extends CI_Model{
	...
	function get_products($data){
		...
		proveedores_sae.nombre AS prov_n');
		...
		$this->db->join('proveedores_sae', 'proveedores_sae.clave = TRIM(products.proveedor)','left');
		//$this->db->join('categories', 'categories.categories_id = products_to_categories.categories_id');
		//$this->db->order_by("orden", "asc"); 
		$this->db->order_by("products_quantity", "desc");
	...
	function search($id_marca, $id_categoria, $products_name, $existencias = FALSE, $ordenar = FALSE){
		...
		switch ($ordenar) {
			...
			case 'ventas3':
				...
				break;	
			case 'ventas6':
				    $order = " ORDER BY p.vmes6 DESC";
				break;	
			case 'ventas12':
				    $order = " ORDER BY p.vmes12 DESC";
				break;					
			case 'precio':
