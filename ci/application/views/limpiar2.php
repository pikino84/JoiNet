  ...
  <head>
	...
	$(function(){
		$('#limpiar').on('click', function(){
			...
			$.ajax({
				url: '/ci/mail/set_mailchimp_batch2',
				type: 'POST',
				data: {'data':$('#contenido').val(),'fk_lista':$('#fk_lista option:selected').val()},
				dataType: 'json',
				success: function(data){
					var tr;
					$.each(data, function(i,v){
						tr += '<tr><td>'+v+' correos agregados.</td></tr>';
					});
					$('#result').html(tr);
				}
			});
            	...
            	<tr>
                	<td>
                        ...
                        <? foreach($lista as $list):?>
                        	<option value="<?=$list->id_lista?>"><?=$list->lista?></option>
                        <? endforeach?>
                        </select>
                    </td>
                </tr>
