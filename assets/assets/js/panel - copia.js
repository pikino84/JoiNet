	...
	$(document).ready(function()
	{
    	...
    	function new_input(nxt_id)
    	{
			var new_input = '<tr id="'+nxt_id+'">';
				...
				new_input += '<td><div id="nombre-'+nxt_id+'"></div></td>';
		...
		function GetProducto(products_id,idp)
		{
			$.ajax({
				...
				$('#precio-'+idp).html('<span class="precio precio_menudeo">$'+ret.products_price+'</span>  <span class="precio precio_mayoreo">$'+ret.precio_mayoreo+'</span>   <span class="precio precio_distribuidor">$'+ret.precio_distribuidor+'</span> ');
		...
		function ObjectJson(q)
		{
			...
			$.ajax({
				...
				{
					list += '<tr class="AddProd" data-id="'+v.codigo+'"><td> <img style="width:75px;" src="'+v.img+'" class="thumbnail" /> </td><td> <strong>'+v.codigo+' </strong>  '+v.desc+' </td></tr>';
				});

				$('.ResultadosProductos').html('<table class="table table-striped table-condensed">'+list+'</table>');

				$('.AddProd').on('dblclick', function(){
